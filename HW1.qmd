...
---
Link to GitHub: `https://github.com/marietolteca00/eds240-hw1-ufo`
...
1.  **What is the author defining in lines 15-17? Where else in the code do these defined variables show up? What advantage(s) is there to defining these values here, as variables, rather than defining the values directly throughout the script?**
    ...
    -   The author is defining in lines 15-17 the colors of the variable called alien, `bg` is set to the first color/ hex code, and `accent` is assigned to the other color/ hex code. When the plots are being made the variables `bg` and `accent` are used. The advantages of assigning the variables, is that when wanting to use the variables made more efficiently.
...
2.  **In your own words, explain what the function, `font_add_google()`, does. What's the difference between the two arguments, `name` and `family`?**
    ...
    -   Font add google allows us to use the font from Google Font repository. The argument `name` is the name of the font that will be serched in Google Fonts. Whereas `family` specifies the font in R.
...
3.  **Describe what this data frame contains.**
    ...
    -   This dataframe contains the states in the 'US' using the country code column from places. When creating the new column state, `str_replace` was used to keep data formatting consistant with acronyms beings all in upper case. Lastly, it was grouped by state and summarise by the sum of population for each state.
...
4.  **Describe what this data frame contains.**
    ...
    -   Here we are using the ufo_sighting dataframe, it has been filtered to only grab the areas in country code column. A new column was made, called state and same code was applied as before to have consistency in all uppercase. Sightings are grouped by state, producing one row per state with the total number of observations. This is joined with population data, allowing calculation of num_obs_per10k, which represents the number of UFO sightings per 10,000 residents in each state.

5.  **What does `opacity_val` represent, and why is it calculated?**

    -   The `opacity_val` represents a normalized measure of UFO sightings per capita, scaled between 0 and 1 by dividing each state’s num_obs_per10k by the maximum value across states. This variable is used for visualization purposes, allowing states with higher per-capita sightings to appear more visually prominent.
...
6.  **Describe what this data frame contains.**
    ...
    -   In this dataframe we have excluded rows with 'unknown' and 'other' in the shape column. The count line is counting how many times the variable appeared in the shape column. The column 'n' is being renamed into 'total_sightings', it is arranged in a descending order (largest to smallest), sliced at 10, to only see the top 10, and lastly we are creating a shape column that is being facet reordered with shape and total sightings. The other column made is opacity value where it is being rescaled to only include values from 0.3-1.
...
7.  **What does `fct_reorder` do when it is applied to the `shape` variable? What would have happened if this step was not performed?**
    ...
    -   `fct_reorder` is setting the order of the variables in shape, by default it will order them from lowest to highest or alphabetical. Without `fct_reorder`, the results will not be in a sequential format and when making a visual it will make interpretation more difficult.

8.  **What is the purpose of rescaling `opacity_val`? And why rescale from 0.3 to 1?**

    -   Rescaling opacity from 0.3 to 1 ensures that all states remain visible in the visualization while preserving relative differences in UFO sightings per capita. Setting a minimum opacity of 0.3 prevents low-value states from becoming invisible, while the maximum opacity of 1 highlights states with the highest per-capita sightings.
...
9.  **Describe what this data frame contains.**
    ...
    -   Columns were created to have specified time stamps (day, hour, workday) using the reported date time column from the ufo sightings. From that a count line was ran to view how many times it events overlapped for the new columns. The count column was renamed to total daily obeservations. More columns are created to get an estimation of 0.0-1, and and hour lab column was made to get an approximation of when events occured during the hour column using statements.

10. **What is the purpose of the last line inside the `case_when()` statement (`TRUE ~ paste0(hour - 12, "pm")`)?**

    -   The last line of case_when is a condition that formats all remaining hours as PM times by converting from 24-hour to 12-hour time. It ensures that any hour not explicitly labeled as AM or noon is displayed as a PM value.
...
11. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
    ...
    -   The difference between `paste0` and `glue` is that glue is able to interpolate and format a string, similar to `paste0` but it can be used in more complex areas. `Paste0` concatenates vectors after converting to characters.
...
12. **Explain the values provided to the `x` aesthetic for both text geoms (`shape` & `total_sightings`).**
    ...
    -   The values provided in the `x` asthetic for both text geoms is to show the variables of the shape column and the total sightings number to be shown on the column plot.
...
13. **Consider the order of `geom_*()` layers in the the above plot (`plot_us`). Why did the author order the layers in this way?**
    ...
    -   In the first geom_rect(), this is making the size of the squares and it is being filled with the opacity values. The following line geom_text() is adding the text into the squares by state along as the type of google font and family. The following line geom_text() is adding the numbers below the state abbreviation, along with the google fonts and family. The author set the layers this way to create a different visualization rather than a plot. The layers had to be set in this format because if they were switched then it would've blocked information. Essentially the way the lines are coded matters.
...
14. **This plot includes one-letter labels for each day of the week. How is ths accomplished when week days are written using their three-letter abbreviations (e.g. `Mon`, `Tue`) in the `df_day_hour` data frame?**
    ...
    -   The labels for each day of the week are being extracted by using str_sub in geom_text, it is only grabbing the first letter instead of the whole word.
...
15. **What role do the `ylim()` and `xlim()` functions play in shaping a ggplot, and how do they change the visual layout of this particular plot? To better understand their effect, try rerunning the code with each of these lines commented out and observe how the plot’s spacing and composition change.**
    ...
    -   When testing different numbers for ylim, i noticed a difference in the way the inner circle of opacity values would be the ones affected. When xlim is changed the times overlap or the circle is not noticable and looks like a half circle with the timestamps overlapping.
...
16. **Why do you think the author chose to convert these text elements (and also in `plot_ufo`, below!) into ggplot objects (you may consider returning to this question after you've worked your way through all of the code)?**
    ...
    -   Turning the quotes into ggplot objects allows them to be composed and aligned with other plots using ggplot based layout tools, making figure assembly more consistent and flexible.
...
17. **Why does the author render `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` (like he does for rendering `plot.title` and `text`)?**
    ...
    -   The author uses `ggtext::element_markdown()` for the plot caption because it supports Markdown based rich text formatting, such as italics, color changes, and hyperlinks within the caption. In contrast, `element_text()` applies a single style and cannot render inline formatting, making it less flexible for captions that include credits or styled text.
...
18. **Explain how `plot_final` is assembled. What do you think is the most challenging aspect of arranging all components into a single plot?**
    ...
    -   Using the previous plot made, 'plot_base', as the name specifies what it is, it will be the main template of how the previous plots created are going to be added. The plot is assembled using inset_element(), to add each plot made, that function has to be used. As well as setting the location of the plots or text.

19. **Can you think of one reason the author may have chosen to separate the construction of `plot_base` and `plot_final`?**

    -   I think it can be used as other base plots for any other infographic or visual. This will help not making duplicate text and make ciding more efficient.
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    a.  **What "perceptual tasks" (from Cleveland & McGill's heirarchy) must the viewer perform to extract information from these visualizations?**
        ...
        -   They emphasized to construct plots based on elementary tasks "as high in the hierchary as possible". Essentially the perceptual task are to make it as easy as possible to the viewer, make it simple and easy to understand. A quick glance and they can extract information from it. Using position along a common scale for comparing counts or observations across states. As well as, Position along identical but non-aligned scales such as comparing or interpreting multiple panels. Color is also another way to show information on visualizations like the `opacity_val` that was used by the author.
    ...
    b.  **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        -   The tasks that the author wanted to convey with the visualizations were too reveal a connection and big picture trends, between the 10 commonly sighted shapes, 10k sights population with states in the US, and days out of the week with time stamps when observations were noted.
    ...
    c.  **Name at least one caveat to the "hierarchy of perceptual tasks" that the author employed to achieve a goal(s) you noted in question b?**
        ...
        -   One caveat the author employed to achieve his goal was the making of the 10k sighting population in the US states. The reason I chose this one was because it reminded me of a periodic table at first, not a visual that is common when representing data. I thought it was pretty cool to create the visual.

21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?**

    -   The two elements would be the bar/ column plot was easy to understand, intuitive, and visually pleasing showing that the lower the bar is the more transparent it gets. Another element that was visually pleasing were the added text using google fonts.

22. **Describe two elements of this piece that you feel could be better presented in a different way. Why?**

    -   Two elements that could be better would be to not make it too transparent which makes some of the plots hard to read. The other element would be for the last plot, make the time stamps less cluttered and more apparent as they are also hard to read.
...
23. **Describe two new things that you learned by interpreting / annotating this code. These could be packages, functions, or even code organizational approaches that you hadn't previously known about or considered.**
    ...
    -   I had to search either on here with the `?` and the function to see what it did or change the code to see what would happen. If the `?` did not have text or very vague I would try google and see what the differences or what the package or function does.
    - The difference between `ggtext::element_markdown()`, rather than `element_text()`. As well as making a plot_base() and adding inset_elements to add different plots made before to create an infographic.  
...
24. **How, if at all, did you use AI tools to help you interpret this code? Describe your approach to using these tools for this assignment. In what ways was consulting the documentation more (or less) helpful than using AI?**
    ...
    -   One way that I partially used AI was when I would do google searched, it would give me a table on how functions were different. Other than that I would click the Rstudio package link and it would show something similar to what the `?` would provide
