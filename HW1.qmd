...
4.  **Describe what this data frame contains.**
    ...
    -   Here we are using the ufo_sighting dataframe, it has been filtered to only grab the areas in country code column. A new column was made, called state and same code was applied as before to have consistency in all uppercase.

5.  **What does `opacity_val` represent, and why is it calculated?**

    -   The `opacity_val` is calculating a percentage number of obeservations and population.
...
8.  **What is the purpose of rescaling `opacity_val`? And why rescale from 0.3 to 1?**
    ...
    -   The purpose of rescaling is to have an opacity values from 0.3-1 instead of having outliers it will keep the information close.
...
10. **What is the purpose of the last line inside the `case_when()` statement (`TRUE ~ paste0(hour - 12, "pm")`)?**
    ...
    -   The purpose of the last line is to get the time past midnight, instead of getting negative numbers it changes into AM time stamps.
...
16. **Why do you think the author chose to convert these text elements (and also in `plot_ufo`, below!) into ggplot objects (you may consider returning to this question after you've worked your way through all of the code)?**
    ...
    -   I believe the author chose to do an annotation instead of a geom text because some elements might be hanlded or plotted differently. In these plots we just got the sentences in text. Versus the o previous plots we were showned the visual differently without sentences they were just a single text.
...
17. **Why does the author render `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` (like he does for rendering `plot.title` and `text`)?**
    ...
    -   There are different uses between plot.caption and element_text, in plot.caption it has more leeway and more paramters to be changed. In element_text, we are limited to its properties. Viewing the plot, using plot.caption, has allowed more leeway and changed the visual rather than just using element_text.
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    a.  **What "perceptual tasks" (from Cleveland & McGill's heirarchy) must the viewer perform to extract information from these visualizations?**
        ...
        -   They emphasized to construct plots based on elementary tasks "as high in the hierchary as possible". Essentially the perceptual task are to make it as easy as possible to the viewer, make it simple, and easy to understand. A quick glance and they can extract information from it.
